...
class EnhancedErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    // SAFE ERROR LOGGING - Wrap all service calls in try-catch to prevent secondary errors
    try {
      // Simple console logging only - avoid DOM manipulation during error handling
      console.error(`[ErrorBoundary:${level}] Error caught:`, error.message)
      console.error('[ErrorBoundary] Stack:', error.stack?.substring(0, 200))
      console.error('[ErrorBoundary] Component stack:', errorInfo.componentStack?.substring(0, 200))
      
      // Track error with monitoring service - wrapped in try-catch
      try {
        ...
        console.warn('[ErrorBoundary] Failed to track error with monitoring service:', monitoringError)
      }

      // Handle error with reliability service - wrapped in try-catch
      try {
        reliabilityService.handleUserFriendlyError(
          error,
          {
        ...
        console.warn('[ErrorBoundary] Failed to handle error with reliability service:', reliabilityError)
      }
    } catch (loggingError) {
      // Even if logging fails, don't crash - just use basic console.error
      console.error('[ErrorBoundary] Error in error handling:', loggingError)
    }

    // Store error info in state
    this.setState({ errorInfo })

    // Call custom error handler if provided - wrapped in try-catch
    if (onError) {
      try {
        onError(error, errorInfo)
      } catch (onErrorError) {
        console.warn('[ErrorBoundary] Custom onError handler failed:', onErrorError)
      }
    }
  ...
  private handleManualRetry = () => {
    // Track manual retry - wrapped in try-catch
    try {
      ...
      console.warn('[ErrorBoundary] Failed to track manual retry:', error)
    }
  ...
  private handleReportError = () => {
    ...
    // Track error report submission - wrapped in try-catch
    try {
      monitoringService.trackBusinessEvent({
      ...
      console.warn('[ErrorBoundary] Failed to track error report:', error)
    }
  ...
  private handleGoHome = () => {
    // Track navigation to home - wrapped in try-catch
    try {
      monitoringService.trackBusinessEvent({
      ...
      console.warn('[ErrorBoundary] Failed to track home navigation:', error)
    }
