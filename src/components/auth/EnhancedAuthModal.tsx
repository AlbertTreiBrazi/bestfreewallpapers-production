...
import { Eye, EyeOff, User, Mail, Lock, Shield, AlertTriangle, Loader2 } from 'lucide-react'
...
export function EnhancedAuthModal({ isOpen, onClose, initialMode = 'login' }: EnhancedAuthModalProps) {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'facebook') => {
    if (!isFeatureEnabled('AUTH_UNIFIED')) {
      toast.error('Social login is not available. Please use email/password.')
      return
    }
      ...
      <div className={`modal-content bg-theme-modal border border-theme-light rounded-lg shadow-xl w-full max-w-[calc(100vw-1rem)] sm:max-w-sm md:max-w-md mx-auto my-auto relative ${theme} safe-area-inset`}>
        ...
        <div className="p-4 sm:p-5 md:p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
            {/* Social Login Options (VITE_AUTH_UNIFIED) */}
            {isFeatureEnabled('AUTH_UNIFIED') && mode !== 'forgot' && (
              <div className="space-y-3">
                <button
                  ...
                  disabled={loading}
                  className="w-full flex items-center justify-center space-x-2 px-4 py-3 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed min-h-[44px] touch-manipulation"
                >
