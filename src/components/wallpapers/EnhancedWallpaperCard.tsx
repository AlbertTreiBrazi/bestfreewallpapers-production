...
export function EnhancedWallpaperCard({
  ...
  // Base image: prefer thumbnail_url if available, otherwise use image_url.
  const baseImage = thumbnail_url || image_url;
  const thumbWidth = variant === 'compact' ? 420 : 640;

  // IMPORTANT:
  // - Only use Supabase /render/image when the input is a public Storage object URL.
  // - Do NOT run /render/image on .webp/.avif inputs (it returns 400 in many setups).
  const canTransform =
  ...
  // Start with the transformed URL (if safe). If it fails, fall back to baseImage.
  const [imgSrc, setImgSrc] = useState<string>(transformedImage);

  // Keep imgSrc in sync when wallpaper changes.
  useEffect(() => {
      ...
      <div className="relative overflow-hidden" style={{ aspectRatio: thumbnailAspectRatio }}>
        ...
        >
          <SafeImage
            ...
            draggable={false}
            onError={() => {
              // If /render/image fails (400), fall back to the original public object URL.
