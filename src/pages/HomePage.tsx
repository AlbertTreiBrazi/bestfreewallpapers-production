            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className={`rounded-xl p-5 border ${theme === 'dark' ? 'bg-dark-secondary border-dark-border' : 'bg-white border-gray-200'}`}>
                ...
                {wallpapers.length > 0 ? (
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    {[...Array(6)].map((_, i) => <div key={i} className={`rounded-lg animate-pulse ${theme === 'dark' ? 'bg-dark-tertiary' : 'bg-gray-100'}`} style={{ aspectRatio: '9/16', maxHeight: '160px' }} />)}
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  {liveWallpapers.length > 0 ? (
                    liveWallpapers.slice(0, 4).map((w: any, i: number) => (
                      <Link key={w.id || i} to="/live-wallpapers" className="relative rounded-xl overflow-hidden group block bg-black" style={{ aspectRatio: '9/16', maxHeight: '160px' }}>
                        {w.video_url ? (
                          <video
                            src={w.video_url}
                    ...
                    [...Array(2)].map((_, i) => (
                      <Link key={i} to="/live-wallpapers" className="relative rounded-xl overflow-hidden group flex items-center justify-center" style={{ aspectRatio: '9/16', maxHeight: '160px', background: i === 0 ? 'linear-gradient(135deg,#1a1a2e,#16213e,#0f3460)' : 'linear-gradient(135deg,#2d1b69,#11998e,#38ef7d)' }}>
