                ...
                return (
                  <button
                    ...
                    className={`${viewMode === 'grid' ? 
                      `group ${theme === 'dark' ? 'bg-dark-secondary hover:bg-dark-tertiary border-dark-border shadow-lg hover:shadow-xl' : 'bg-white hover:bg-gray-50 border-gray-200 shadow-sm hover:shadow-md'} border rounded-xl overflow-hidden transition-all duration-300 hover:scale-105 relative` :
                      ...
                      <>
                        ...
                        <div className="p-2.5 sm:p-3">
                          <div className="flex items-center justify-between mb-1.5">
                            <h3 className={`font-semibold ${theme === 'dark' ? 'text-white group-hover:text-gray-400' : 'text-gray-900 group-hover:text-gray-600'} transition-colors duration-200 text-xs sm:text-sm leading-tight line-clamp-1 flex-1`}>
                              {category.name}
                            </h3>
                            {/* Count badge */}
                            <div className={`ml-2 px-1.5 py-0.5 rounded-full text-xs font-medium ${theme === 'dark' ? 'bg-purple-900/50 text-purple-300' : 'bg-gray-100 text-gray-600'}`}>
                              {(category.wallpaper_count || 0) > 999 ? `${Math.floor((category.wallpaper_count || 0) / 100) / 10}k` : (category.wallpaper_count || 0)}
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-1 mb-1.5">
                            {/* Hot/Featured badges */}
                            {(category.name.toLowerCase().includes('trending') || category.name.toLowerCase().includes('popular')) && (
                              <span className="px-1.5 py-0.5 rounded-full text-xs font-medium bg-gradient-to-r from-orange-500 to-red-500 text-white">
                                🔥 Hot
                              </span>
                            )}
                            {(category.name.toLowerCase().includes('new') || category.name.toLowerCase().includes('latest')) && (
                              <span className="px-1.5 py-0.5 rounded-full text-xs font-medium bg-gradient-to-r from-blue-500 to-cyan-500 text-white">
                                🆕 New
                              </span>
                            )}
                            {(category.name.toLowerCase().includes('premium') || category.name.toLowerCase().includes('pro')) && (
                              <span className="px-1.5 py-0.5 rounded-full text-xs font-medium bg-gradient-to-r from-yellow-500 to-orange-500 text-white">
                                ⭐ Pro
                              </span>
                            )}
                            {category.wallpaper_count && category.wallpaper_count > 100 && (
                              <span className="px-1.5 py-0.5 rounded-full text-xs font-medium bg-gradient-to-r from-green-500 to-emerald-500 text-white">
                                ✅ Curated
                              </span>
                            )}
                          </div>
                          ...
                          <p className={`text-xs ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} transition-colors duration-200`}>
                            {(category.wallpaper_count || 0) === 1 ? '1 wallpaper' : `${category.wallpaper_count || 0} wallpapers`}
                          </p>
                        </div>
                        <div className="absolute top-2 right-2 opacity-70" title="Dedicated page">
                          <div className={`w-5 h-5 rounded-full ${theme === 'dark' ? 'bg-dark-primary' : 'bg-white'} flex items-center justify-center text-xs border ${theme === 'dark' ? 'border-gray-600' : 'border-gray-300'}`}>
                            📄
                          </div>
                        </div>
                      </>
