...
export function AuthPage() {
  ...
  // VITE_AUTH_UNIFIED: Redirect to /login?showModal=1 when feature flag is enabled
  useEffect(() => {
    if (isFeatureEnabled('AUTH_UNIFIED')) {
      // Preserve all query parameters during redirect
      const redirect = searchParams.get('redirect')
      const mode = searchParams.get('mode')
      const params = new URLSearchParams()
      
      // Always add showModal=1 to open the modal automatically
      params.set('showModal', '1')
