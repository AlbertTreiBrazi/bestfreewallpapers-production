...
function getBaseHtml(): string {
  ...
  } catch (e) { console.error('[SEO Live Wallpaper] index.html error:', e); }
...
async function fetchLiveWallpaper(slug: string): Promise<LiveWallpaperData | null> {
  ...
  try {
    ...
    console.error('[SEO Live Wallpaper] fetch error:', e);
    return null;
  }
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!wallpaper) {
    // Fallback pentru wallpaper negăsit
    title = 'Live Wallpaper | BestFreeWallpapers';
    description = 'Download free animated live wallpapers for your phone.';
  ...
  `;
    ...
    description = wallpaper.description
      || `Download ${wallpaper.title} free animated live wallpaper for your phone. High-quality MP4 video wallpaper, no watermark, no signup required.`;
    keywords = [
      'free live wallpaper',
    ...
    ogImage = wallpaper.thumbnail_url || OG_IMAGE_DEFAULT;

    // VideoObject JSON-LD — important pentru Google Video Search
    const jsonLd = [
      {
        ...
        interactionStatistic: {
          ...
          userInteractionCount: wallpaper.views_count || 0,
        },
        publisher: {
          '@type': 'Organization',
  ...
  res.setHeader('Cache-Control', wallpaper
