...
async function handleStatic(page: string, res: VercelResponse) {
  ...
  ${firstWallpaperUrl ? `<link rel="preload" as="image" href="${firstWallpaperUrl}" fetchpriority="high" />` : ''}
  `;
  const baseHtml = getBaseHtml();
  const withHead = injectHead(baseHtml, tags);
  // NU mai pre-randam body-ul. Pre-render-ul includea doar hero + tabs,
  // dar NU Header + PremiumBanner → React le insera deasupra la hydration
  // → shift de ~174px → CLS 0.331.
  // Meta tags (title/description/og/canonical/jsonLd) raman in <head> pentru SEO.
  // Googlebot randeaza JS oricum, deci continutul e indexat normal.
  const finalHtml = withHead;
