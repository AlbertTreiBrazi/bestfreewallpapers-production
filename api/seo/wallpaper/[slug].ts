...
<body style="background-color: #ffffff; color: #1f2937; margin: 0; padding: 0;">
  ...
  <script type="module">
    // Dynamic app loader - discovers and loads the Vite-built entry point
    (async function() {
      try {
        // Fetch the homepage to extract the actual script path
        const res = await fetch('/?_seo_loader=1');
        const html = await res.text();

        // Extract the module script src from the built HTML
        ...
        if (scriptMatch && scriptMatch[1]) {
          ...
          // Fallback: redirect to let the SPA handle it
          console.warn('Could not find module script, falling back');
        }

        // Also load any stylesheet links
...
export default async function handler(
  ...
  try {
    ...
    if (!wallpaper) {
      // Wallpaper not found - return 404
      response.status(404);
...
<body>
  ...
  <a href="/free-wallpapers">Browse Wallpapers</a>
    ...
    // On error, return a basic error page
    response.status(500);
    response.setHeader('Content-Type', 'text/html');
    response.send(`<!doctype html>
<html lang="en">
